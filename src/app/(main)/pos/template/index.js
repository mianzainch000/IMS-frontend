...
const POSPage = () => {
  ...
  const handleDiscountChange = (id, value, type) => {
    setCart((prev) =>
      prev.map((item) => {
        if (item._id === id) {
          const discountVal = value === "" ? 0 : parseFloat(value);
  ...
  return (
    <div className={styles.posContainer}>
      ...
      { }
      <div className={`${styles.printableBill} printableBill`}>

        {/* HEADER */}
        <div className={`${styles.billHeader} billHeader`}>
          <h2>MERA STORE</h2>
          <p>Samundri, Punjab</p>
        ...
        {/* TABLE */}
        <table className={`${styles.billTable} billTable`}>
          <thead>
            <tr>
              <th>Item</th>
              <th>Qty</th>
        ...
        {/* FOOTER */}
        <div className={`${styles.billFooter} billFooter`}>
